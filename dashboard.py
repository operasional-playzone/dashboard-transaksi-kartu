...
def format_rupiah(nilai):
    """Mengubah angka menjadi format Rp 1.000.000 (Format Indonesia)"""
    return f"Rp {nilai:,.0f}".replace(',', '.')

def format_singkat_id(nilai):
    """Mengubah angka besar menjadi singkatan Indonesia (Jt, M)"""
...
def load_data():
    ...
    try:
        ...
        # 2. Bersihkan String (Tambahkan Folder_Asal)
        # REVISI: Menambahkan 'Folder_Asal' ke pembersihan string
...
if df_raw is not None:
    ...
    def create_filter(label, column_name):
        # Pastikan kolom ada sebelum mengambil unique
    ...
    # REVISI: Menggunakan Folder_Asal sebagai filter Toko
    ...
    # Filter Kategori (REVISI: Filter ke kolom Folder_Asal)
    ...
    # REVISI: Menghitung Toko Aktif berdasarkan Folder_Asal
    ...
    with tab2:
        ...
        if not df_filtered.empty:
            ...
            with col_rank_1:
                ...
                fig_top_p = px.bar(
                    df_top_paket, x='Omset_Paket', y='Paket', orientation='h',
                    title="Top 10 Paket by Omset",
                    text='Label_Bar', custom_data=['Label_Tooltip'],
                    color_discrete_sequence=['#3366CC'] 
                )
                
                fig_top_p.update_traces(
                    hovertemplate="<b>%{y}</b><br>Omset: %{customdata[0]}<extra></extra>",
                    textposition='outside'
                )
                fig_top_p.update_layout(
                    yaxis={'title': ''}, 
                    xaxis={'title': 'Total Omset (Rupiah)', 'showticklabels': False},
                    showlegend=False
                )
                st.plotly_chart(fig_top_p, use_container_width=True)
                
            # CHART 2: Top 10 Toko (REVISI: Berdasarkan Folder_Asal)
            with col_rank_2:
                st.markdown("#### 🏪 Top 10 Toko Terbaik")
                # Group by Folder_Asal
                ...
                fig_top_t = px.bar(
                    df_top_toko, 
                    x='Omset_Paket', 
                    # Tampilkan Nama Toko (Folder)
                    y='Folder_Asal', 
                    orientation='h',
                    title="Top 10 Toko by Omset",
                    text='Label_Bar', 
                    custom_data=['Label_Tooltip'],
                    color_discrete_sequence=['#109618'] 
                )
                
                fig_top_t.update_traces(
                    hovertemplate="<b>%{y}</b><br>Omset: %{customdata[0]}<extra></extra>",
                    textposition='outside'
                )
                fig_top_t.update_layout(
                    yaxis={'title': ''}, 
                    xaxis={'title': 'Total Omset (Rupiah)', 'showticklabels': False},
                    showlegend=False
                )
    ...
    with tab3:
        ...
        if not df_filtered.empty:
            df_scatter = df_filtered.groupby(['Paket', 'Kategori_Paket']).agg({
                'Omset_Paket': 'sum',
                'Frekuensi': 'sum'
            }).reset_index()
            ...
            fig_scatter = px.scatter(
                df_scatter,
                x='Frekuensi', y='Omset_Paket',
                size='Frekuensi', color='Kategori_Paket',
                hover_name='Paket',
                custom_data=['Label_Omset', 'Label_Harga'],
                log_x=True, log_y=True,
                title="Sebaran Paket: Frekuensi vs Omset (Log Scale)"
            )
            # Tooltip Kustom
            fig_scatter.update_traces(
                hovertemplate="<b>%{hovertext}</b><br>" +
                              "Omset: %{customdata[0]}<br>" +
                              "Transaksi: %{x}<br>" +
                              "Rata-rata Harga: %{customdata[1]}"
            )
