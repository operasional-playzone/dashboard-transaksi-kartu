...
# ================= 3. HELPER FUNCTIONS =================
# ... (BAGIAN HELPER TETAP SAMA, TIDAK PERLU DIUBAH) ...
...
# ================= 4. LOAD DATA (VIA GSPREAD) =================

# --- URL GOOGLE SHEETS ANDA ---
# Mengambil Link dari secrets.toml
try:
    URL_KARTU = st.secrets["spreadsheet_links"]["url_kartu"]
    URL_MESIN = st.secrets["spreadsheet_links"]["url_mesin"]
except Exception as e:
    st.error("Gagal membaca Link Spreadsheet dari secrets.toml. Pastikan formatnya benar.")
    st.stop()

# Fungsi Koneksi GSpread (Cached Resource)
@st.cache_resource
def get_gspread_client():
    # Mengambil kredensial dari secrets.toml
    scope = ['https://www.googleapis.com/auth/spreadsheets', 'https://www.googleapis.com/auth/drive']
    credentials = Credentials.from_service_account_info(
        st.secrets["gcp_service_account"], # Pastikan nama header di secrets.toml sama
        scopes=scope
    )
...
# Fungsi Load Data Mesin (Cached Data)
@st.cache_data(ttl=600)
def load_data_mesin():
    try:
        client = get_gspread_client()
